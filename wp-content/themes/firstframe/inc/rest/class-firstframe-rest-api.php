...
if ( ! class_exists( 'Firstframe_Rest_API' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class Firstframe_Rest_API {
		private static $instance;
		private $namespace;

		public function __construct() {
			// Init variables
			$this->set_namespace( 'firstframe/v1' );

			// Localize theme's main js script with rest variable
			add_filter( 'firstframe_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'firstframe_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		function register_rest_api_route() {
			$routes = apply_filters( 'firstframe_filter_rest_api_routes', array() );
	...
	Firstframe_Rest_API::get_instance();
}

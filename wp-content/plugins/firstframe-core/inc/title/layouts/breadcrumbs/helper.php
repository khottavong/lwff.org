...
if ( ! function_exists( 'firstframe_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'FirstFrameCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'firstframe_core_filter_register_title_layouts', 'firstframe_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'firstframe_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'firstframe-core' );

		return $layouts;
	}

	add_filter( 'firstframe_core_filter_title_layout_options', 'firstframe_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'firstframe_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function firstframe_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'firstframe_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'firstframe-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'firstframe-core' ),
				'author'      => esc_html__( 'Posted by %s', 'firstframe-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'firstframe-core' ),
				'404'         => esc_html__( '404 - Page not found', 'firstframe-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'firstframe-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'firstframe_core_filter_breadcrumbs_settings',
			array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'firstframe_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'firstframe_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}

...
class FirstFrameCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'firstframe_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'firstframe_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return FirstFrameCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( FIRSTFRAME_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( FIRSTFRAME_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = firstframe_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'firstframe_core_filter_register_title_layouts', array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'firstframe_filter_title_template' : 'firstframe_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = firstframe_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = firstframe_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = firstframe_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = firstframe_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height                   = firstframe_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = firstframe_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding            = apply_filters( 'firstframe_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'firstframe_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = firstframe_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = firstframe_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = firstframe_core_get_page_title_image_params();
		...
		$title_color = firstframe_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
FirstFrameCore_Titles::get_instance();

...
abstract class FirstFrameCore_List_Shortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'firstframe_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => firstframe_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => firstframe_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
					'default_value' => '3',
					'group'         => $group,
					'dependency'    => array(
						'hide' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Row Height', 'firstframe-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Max Row Height', 'firstframe-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Last Row Treshold', 'firstframe-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option(
			array(
				'field_type'  => 'text',
				'name'        => 'slider_speed',
				'title'       => esc_html__( 'Slide Duration', 'firstframe-core' ),
				'description' => esc_html__( 'Default value is 5000 (ms)', 'firstframe-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		$this->set_option(
			array(
				...
				'title'       => esc_html__( 'Slide Animation Duration', 'firstframe-core' ),
				'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'firstframe-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		...
		$this->set_option(
			array(
				...
				'title'         => esc_html__( 'Choose Slide Animation', 'firstframe-core' ),
				'options'       => array(
					''     => esc_html__( 'Default (Slide)', 'firstframe-core' ),
					'fade' => esc_html__( 'Fade', 'firstframe-core' ),
				),
				'default_value' => '',
				'description'   => esc_html__( 'Number of columns will be set to one.', 'firstframe-core' ),
				'dependency'    => $dependency,
				'group'         => $group,
			)
		);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_direction', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Choose Slide Direction', 'firstframe-core' ),
					'options'       => array(
						''         => esc_html__( 'Default (Horizontal)', 'firstframe-core' ),
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_hidden_slides', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Show Hidden Slides', 'firstframe-core' ),
					'options'     => firstframe_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'Works best with horizontal slide direction and slide animation.', 'firstframe-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Loop Additional Slides', 'firstframe-core' ),
					'options'     => firstframe_core_get_select_type_options_pool( 'yes_no' ),
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_centered_slides', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Enable Centered Slides', 'firstframe-core' ),
					'options'     => firstframe_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'If true, then active slide will be centered, not always on the left side.', 'firstframe-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_mousewheel_navigation', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Enable Slider Mousewheel Navigation', 'firstframe-core' ),
					'options'     => firstframe_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'Enables navigation through slides using mouse wheel.', 'firstframe-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Show Slider Navigation', 'firstframe-core' ),
				'options'    => firstframe_core_get_select_type_options_pool( 'yes_no' ),
				'dependency' => $dependency,
				'group'      => $group,
			)
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Show Slider Pagination', 'firstframe-core' ),
				'options'    => firstframe_core_get_select_type_options_pool( 'yes_no' ),
				'dependency' => $dependency,
				'group'      => $group,
			)
		...
		$this->set_option(
			array(
				'field_type'    => 'select',
				'name'          => 'split_slider',
				'title'         => esc_html__( 'Enable Split Slider', 'firstframe-core' ),
				'options'       => firstframe_core_get_select_type_options_pool( 'yes_no' ),
				'default_value' => '',
				'dependency'    => array(
					'show' => array(
		...
		$this->set_option(
			array(
				'field_type'    => 'text',
				'name'          => 'split_number',
				'title'         => esc_html__( 'Enter Split number', 'firstframe-core' ),
				'options'       => firstframe_core_get_select_type_options_pool( 'yes_no' ),
				'default_value' => '3.3',
				'dependency'    => array(
					'show' => array(
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'masonry' === $atts['behavior'] && ! empty( $atts['masonry_images_proportion'] ) && 'fixed' === $atts['masonry_images_proportion'] ) {
			...
			$image_dimension              = firstframe_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']         = isset( $atts['space'] ) ? firstframe_core_get_space_value( $atts['space'] ) : 0;
		...
		$data['imagesOnTheSide']      = isset( $atts['imagesOnTheSide'] ) ? $atts['imagesOnTheSide'] : '';

		if ( $atts['split_slider'] === 'yes' ) {
			$data['slidesPerView'] = $atts['split_number'];
		}
		if ( ! empty( $atts['columns_responsive'] ) && 'custom' === $atts['columns_responsive'] ) {
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'firstframe-core' );
		...
		$this->set_option(
			array(
				'field_type'    => 'text',
				'name'          => 'posts_per_page',
				'title'         => esc_html__( 'Posts per Page', 'firstframe-core' ),
				'default_value' => '9',
				'group'         => $group,
			)
		);
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'orderby', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'orderby',
					'title'         => esc_html__( 'Order By', 'firstframe-core' ),
					'options'       => firstframe_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
					'default_value' => 'date',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'order', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'order',
					'title'         => esc_html__( 'Order', 'firstframe-core' ),
					'options'       => firstframe_core_get_select_type_options_pool( 'order' ),
					'default_value' => 'DESC',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option, true ) ) ) {
			...
			do_action( 'firstframe_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters(
				'firstframe_core_filter_map_additional_query_params',
				array(
					''       => esc_html__( 'No', 'firstframe-core' ),
					'id'     => esc_html__( 'Post IDs', 'firstframe-core' ),
					'tax'    => esc_html__( 'Taxonomy Slug', 'firstframe-core' ),
					'author' => esc_html__( 'Author Name', 'firstframe-core' ),
				),
				$this->get_base()
			);
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'post_ids',
					'title'       => esc_html__( 'Posts IDs', 'firstframe-core' ),
					'description' => esc_html__( 'Separate post IDs with commas', 'firstframe-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				...
				$this->set_option(
					array(
						'field_type' => 'text',
						'name'       => 'tax_slug',
						'title'      => esc_html__( 'Taxonomy Slug', 'firstframe-core' ),
						'group'      => $group,
						'dependency' => array(
							'show' => array(
								'additional_params' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'author_slug',
					'title'      => esc_html__( 'Author Slug', 'firstframe-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			do_action( 'firstframe_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'firstframe_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option(
			array(
				...
				'options'       => $layouts,
				'default_value' => apply_filters( 'firstframe_core_filter_map_layout_options_default_value', $default_value, $this->get_base(), $params ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'text_transform',
					'title'      => esc_html__( 'Title Text Transform', 'firstframe-core' ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'firstframe-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_filter',
					'title'         => esc_html__( 'Enable Filter', 'firstframe-core' ),
					'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'firstframe-core' ),
			...
			do_action( 'firstframe_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Load More Top Margin (px or %)', 'firstframe-core' ),
					'default_value' => '',
					'group'         => $group_name,
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'firstframe_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function load_assets() {
		do_action( 'firstframe_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}

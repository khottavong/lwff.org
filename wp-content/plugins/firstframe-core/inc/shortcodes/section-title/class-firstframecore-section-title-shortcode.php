...
if ( ! function_exists( 'firstframe_core_add_section_title_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_add_section_title_shortcode( $shortcodes ) {
		$shortcodes[] = 'FirstFrameCore_Section_Title_Shortcode';

		return $shortcodes;
	}

	add_filter( 'firstframe_core_filter_register_shortcodes', 'firstframe_core_add_section_title_shortcode' );
}

if ( class_exists( 'FirstFrameCore_Shortcode' ) ) {
	class FirstFrameCore_Section_Title_Shortcode extends FirstFrameCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( FIRSTFRAME_CORE_SHORTCODES_URL_PATH . '/section-title' );
			$this->set_base( 'firstframe_core_section_title' );
			$this->set_name( esc_html__( 'Section Title', 'firstframe-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds section title element', 'firstframe-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'link',
					'title'      => esc_html__( 'Title Custom Link', 'firstframe-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return firstframe_core_get_template_part( 'shortcodes/section-title', 'templates/section-title', '', $atts );
		}

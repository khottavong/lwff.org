...
if ( ! function_exists( 'firstframe_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'FirstFrameCore_Image_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'firstframe_core_filter_register_shortcodes', 'firstframe_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'FirstFrameCore_Shortcode' ) ) {
	class FirstFrameCore_Image_Marquee_Shortcode extends FirstFrameCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'firstframe_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'firstframe_core_filter_image_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( FIRSTFRAME_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'firstframe_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'firstframe-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'firstframe-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'duration',
					'title'         => esc_html__( 'Animation Duration (Seconds)', 'firstframe-core' ),
					'default_value' => '20',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return firstframe_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}

...
if ( ! function_exists( 'firstframe_core_add_custom_font_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_add_custom_font_shortcode( $shortcodes ) {
		$shortcodes[] = 'FirstFrameCore_Custom_Font_Shortcode';

		return $shortcodes;
	}

	add_filter( 'firstframe_core_filter_register_shortcodes', 'firstframe_core_add_custom_font_shortcode' );
}

if ( class_exists( 'FirstFrameCore_Shortcode' ) ) {
	class FirstFrameCore_Custom_Font_Shortcode extends FirstFrameCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'firstframe_core_filter_custom_font_layouts', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( FIRSTFRAME_CORE_SHORTCODES_URL_PATH . '/custom-font' );
			$this->set_base( 'firstframe_core_custom_font' );
			$this->set_name( esc_html__( 'Custom Font', 'firstframe-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays custom font with provided parameters', 'firstframe-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'textarea',
					'name'          => 'title',
					'title'         => esc_html__( 'Title Text', 'firstframe-core' ),
					'default_value' => esc_html__( 'Custom Title Text', 'firstframe-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_outlined_text',
					'title'         => esc_html__( 'Enable Outlined Text', 'firstframe-core' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'firstframe_core_custom_font', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return firstframe_core_get_template_part( 'shortcodes/custom-font', 'variations/' . $atts['layout'] . '/templates/custom-font', '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty( $atts['enable_outlined_text'] ) && 'yes' === $atts['enable_outlined_text'] ? 'qodef-enable-outlined-text--yes' : 'qodef-enable-outlined-text--no';

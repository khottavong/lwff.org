...
if ( ! function_exists( 'firstframe_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'FirstFrameCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'firstframe_core_filter_register_shortcodes', 'firstframe_core_add_highlight_shortcode' );
}

if ( class_exists( 'FirstFrameCore_Shortcode' ) ) {
	class FirstFrameCore_Highlight_Shortcode extends FirstFrameCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( FIRSTFRAME_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'firstframe_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'firstframe-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'firstframe-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'firstframe-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'firstframe-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'firstframe-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'firstframe-core' ),
					'group'       => esc_html__( 'Text Style', 'firstframe-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'firstframe-core' ),
					'group'      => esc_html__( 'Text Style', 'firstframe-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'firstframe-core' ),
					'group'      => esc_html__( 'Text Style', 'firstframe-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return firstframe_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}

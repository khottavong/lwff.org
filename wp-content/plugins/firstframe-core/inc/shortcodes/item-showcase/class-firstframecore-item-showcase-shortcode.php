...
if ( ! function_exists( 'firstframe_core_add_item_showcase_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_core_add_item_showcase_shortcode( $shortcodes ) {
		$shortcodes[] = 'FirstFrameCore_Item_Showcase_Shortcode';

		return $shortcodes;
	}

	add_filter( 'firstframe_core_filter_register_shortcodes', 'firstframe_core_add_item_showcase_shortcode' );
}

if ( class_exists( 'FirstFrameCore_Shortcode' ) ) {
	class FirstFrameCore_Item_Showcase_Shortcode extends FirstFrameCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'firstframe_core_filter_item_showcase_layouts', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( FIRSTFRAME_CORE_SHORTCODES_URL_PATH . '/item-showcase' );
			$this->set_base( 'firstframe_core_item_showcase' );
			$this->set_name( esc_html__( 'Item Showcase', 'firstframe-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds item showcase holder', 'firstframe-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Image Top Offset', 'firstframe-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return firstframe_core_get_template_part( 'shortcodes/item-showcase', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}

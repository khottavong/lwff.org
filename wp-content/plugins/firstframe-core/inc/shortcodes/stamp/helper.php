...
if ( ! function_exists( 'firstframe_str_split_unicode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function firstframe_str_split_unicode( $str ) {
		return preg_split( '~~u', html_entity_decode( $str ), - 1, PREG_SPLIT_NO_EMPTY );
	}
}

if ( ! function_exists( 'firstframe_get_split_text' ) ) {
	/**
	 ...
	 */
	function firstframe_get_split_text( $text ) {
		if ( ! empty( $text ) ) {
			$split_text = firstframe_str_split_unicode( $text );
